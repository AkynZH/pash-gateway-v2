...
function parseStream(stream, schemaResolver) {
  ...
  for (const rawLine of lines) {
    ...
    // FIX (audit): Strict enforcement of v:2 header
    // Version header MUST be the very first non-empty line.
    if (!headerChecked) {
      headerChecked = true;
      ...
      if (vm) {
        ...
        if (version !== '2') {
          ops.push({
            type:   'ERROR',
            level:  'error',
            params: { id: 'stream', component: 'Protocol', reason: `unsupported_version_${version}_expected_2` },
          });
        }
        continue;
      } else {
        // First non-empty line is not a version header — emit strict error
        ops.push({
          type:   'ERROR',
          level:  'error',
          params: { id: 'stream', component: 'Protocol', reason: 'missing_v2_header' },
        });
        // Continue parsing to gather all possible errors, but stream is invalid
      }
    }
